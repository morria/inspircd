...
static const char * S3Host     = "s3.amazonaws.com";     /// <AWS S3 host
/// \todo Use SQSHost in SQS functions
static const char * SQSHost  = "queue.amazonaws.com";  /// <AWS SQS host
...
static size_t writefunc ( char * ptr, size_t size, size_t nmemb, IOBuf *stream )
{
...
static size_t readfunc ( char * ptr, size_t size, size_t nmemb, IOBuf * stream )
{
  char * Ln = ptr;
...
/// \param stream pointer to I/O buffer
/// \return number of bytes processed
static size_t header ( char * ptr, size_t size, size_t nmemb, IOBuf * stream )
{
  IOBuf * b = stream;
...
int aws_read_config ( char * const id )
{
  ...
  FILE * f = __aws_getcfg();
  if ( f == NULL ) { perror ("aws4c: Error opening config file"); exit(1); }
